...
async function readBody(req){let bytes=0;const parts=[];for await(const chunk of req){bytes+=chunk.length;if(bytes>MAX_BODY)throw Error('Request too large');parts.push(chunk);}const value=JSON.parse(Buffer.concat(parts).toString('utf8')||'{}');if(!value||Array.isArray(value)||typeof value!=='object')throw Error('Invalid request');return value;}
